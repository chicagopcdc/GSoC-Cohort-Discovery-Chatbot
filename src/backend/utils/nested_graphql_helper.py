...
def validate_filter_fields(filter_obj, valid_pcdc_fields: set, valid_gitops_paths: set) -> list:
    """
    Recursively walk a generated GraphQL filter and collect invalid field references.

    Checks two things:
      1. Field names inside operators (IN, GTE, LTE, GT, LT) must exist in the
         PCDC schema. Source: processed_pcdc_schema_prod.json maps enum VALUES →
         list of FIELD NAMES (e.g. {"Metastatic": ["tumor_classification", ...]}).
         We extract the field names (the list items, not the keys) to build the
         valid field set.

      2. 'path' values inside nested blocks must be valid GitOps table nodes.
         Source: processed_gitops.json maps FIELD NAMES → list of TABLE NODES
         (e.g. {"tumor_classification": ["tumor_assessments"]}). We extract the
         table node names (the list items) to build the valid path set.

    Args:
        filter_obj:         The filter dict/list to validate (from LLM output).
        valid_pcdc_fields:  Set of all known PCDC field names.
        valid_gitops_paths: Set of all known GitOps table node names.

    Returns:
        List of (bad_value, reason) tuples. Empty list means the filter is clean.
    """
    errors = []

    if isinstance(filter_obj, list):
        for item in filter_obj:
            errors.extend(validate_filter_fields(item, valid_pcdc_fields, valid_gitops_paths))

    elif isinstance(filter_obj, dict):
        for key, value in filter_obj.items():

            if key in ("AND", "OR"):
                # Logical combinators — recurse into the sub-filter list
                errors.extend(validate_filter_fields(value, valid_pcdc_fields, valid_gitops_paths))

            elif key in ("IN", "GTE", "LTE", "GT", "LT"):
                # All comparison/membership operators wrap {field_name: value}.
                # GT/LT are generated by query_builder.py; GTE/LTE are the
                # standard PCDC Guppy operators. Validate the field name for all.
                if isinstance(value, dict):
                    for field_name in value:
                        if field_name not in valid_pcdc_fields:
                            errors.append((
                                field_name,
                                f"Unknown PCDC field '{field_name}' used in {key} operator"
                            ))

            elif key == "nested":
                # Nested block must declare a valid 'path' (a GitOps table node)
                if isinstance(value, dict):
                    path = value.get("path", "")
                    if path and path not in valid_gitops_paths:
                        errors.append((
                            path,
                            f"Unknown nested path '{path}' — not a valid GitOps table node"
                        ))
                    # Recurse into the nested AND/OR conditions
                    for sub_key, sub_val in value.items():
                        if sub_key in ("AND", "OR"):
                            errors.extend(validate_filter_fields(sub_val, valid_pcdc_fields, valid_gitops_paths))
